...
function hashSource(vertex: string, fragment: string): string {
...
function parseShaderLog(log: string, type: ShaderError['type']): ShaderError[] {
...
function ensurePrecision(source: string): string {
...
export function createShaderCompiler(backend: BackendContext): ShaderCompilerAPI {
  const cache = new Map<string, CompiledShader>();
  const allErrors: ShaderError[] = [];

  function compileWebGL(name: string, program: ShaderProgram): CompiledShader {
    const gl = backend.gl;
    if (!gl) {
      const err: ShaderError = { type: 'link', message: 'No WebGL context available' };
      return {
        name,
        program,
